...
QByteArray Serializer::SerializerPrivate::serialize( const QVariant &v, bool *ok, int indentLevel)
{
    ...
    if ( ! v.isValid() ) { // invalid or null?
        str = "null";
    } else if (( v.type() == QVariant::List ) || ( v.type() == QVariant::StringList ) || v.canConvert(QMetaType::QVariantList)){ // an array or a stringlist?
        ...
        if (!(( v.type() == QVariant::List ) || ( v.type() == QVariant::StringList ))){
            ...
            Q_FOREACH( QObject* object, list )
            {
                ...
                for(int i = object->metaObject()->propertyOffset() ; i < object->metaObject()->propertyCount() ; i++){
                    QString property = QString::fromLatin1(object->metaObject()->property(i).name());
                    QVariant value = object->property(property.toLatin1());
                    map.insert(property, value);
                }
                serializedValue = serialize( map, ok, indentLevel+1);
        ...
        str = serialize( map, ok, indentLevel+1);
    } else {
        ...
        if (( v.type() == QVariant::String ) ||  ( v.type() == QVariant::ByteArray )) { // a string or a byte array?
            ...
            const bool special = std::isnan(value) || !std::isfinite(value);
